...
- `lib/aggregate_sessions.py` — roll the per-session counts up across *all* the analyzed sessions into one deterministic `aggregate` object (totals; summed `toolUseByName`/`toolErrorsByName`; how many sessions hit each thrash signal; the most re-read and most-churned files; the worst sessions by error count). Feed it the session paths as positional args or newline-separated on stdin. **Use this for every cross-session count instead of summing footers by eye** — see Step 3. Flag: `--top N` (cap on the per-file / per-session evidence lists, default 25).
...
Then run the aggregator **once** over the same (≤25) selected session paths to get the cross-session rollup — the ground truth for every cross-session count in Step 3. Pass the paths on stdin or as positional args:

    printf '%s\n' "$SELECTED_PATHS" | python3 /Users/discordwell/Projects/personal-tools/session-coach/skill/lib/aggregate_sessions.py

Read its single `aggregate` line. Empty/missing sessions are skipped automatically; its `sessions` field is the number actually rolled up, and the stderr `note:` reports any skips.
...
**Use the mechanical counts, don't re-count them by eye — the `aggregate` line for cross-session totals, each session's `stats` footer for per-session detail.** The per-session footer gives exact, full-session counts (computed before any event elision): `userMsgs`, `interrupts`, `assistantTexts`, `toolCalls`, `toolErrors`, `maxErrorRun` (longest run of consecutive failing tool calls → retry loop), `toolUseByName`, `toolErrorsByName`, `filesReadGt3` (files Read >3× → re-read thrash), and `editRunsByFile` (files with ≥2 consecutive Edits → edit churn). The `aggregate` line rolls those up: `totals`, summed `toolUseByName`/`toolErrorsByName`, `sessionsWithBackToBackFailure`/`sessionsWithRetryLoop`/`sessionsWithReReadThrash`/`sessionsWithEditChurn` (how many sessions hit each signal), `reReadFiles`/`editChurnFiles` (the most-repeated files, with the session count and intensity), and `sessionsByErrors` (the worst sessions, for evidence). Treat both as ground truth for counting; let the events supply the surrounding narrative/quotes.

- **Thrashing** — read `editChurnFiles` (edit churn), `reReadFiles` (re-read loops), and `sessionsWithRetryLoop`/`sessionsWithBackToBackFailure` straight from the aggregate; drill into a specific session's `maxErrorRun` / `editRunsByFile` / `filesReadGt3` footer for detail. When a session's `maxErrorRun` is high, scan its events to see *which* tool looped and *why* (corroborate the tool with `toolErrorsByName`).
- **Tool confusion** — the aggregate's summed `toolErrorsByName` (with `toolUseByName` as the denominator) tells you which tool is most error-prone across all sessions; then read the surrounding events to judge *why* (e.g. `tool_result.ok=false` followed by a different tool succeeding, or Bash `cat`/`head`/`tail`/`sed`/`awk`/`echo > file` used to inspect/write a file when Read/Edit/Write would have worked). The counts deliberately do **not** flag the Bash-instead-of-Read pattern — a regex for it misfires on most Bash commands (pipes to `head`/`tail`, `cat <<EOF` heredocs), so read the command and judge it yourself.
...
A pattern is reportable only if it appears across ≥2 distinct sessions OR is a single high-severity instance (e.g. a clear pushback with explicit frustration). The aggregate's `sessions*` counts and the per-file `sessions` fields make the ≥2-session test mechanical for thrash/tool signals. When citing a count in the journal, take it from the aggregate (cross-session) or a footer (single-session) so it is reproducible.
